...
import random
from .config import WECHAT_APPID, WECHAT_MCH_ID, WECHAT_MCH_KEY, WECHAT_NOTIFY_URL, TRADE_TYPE, BODY_DESC
...
def paysign(appid, body, mch_id, nonce_str, notify_url, openid, out_trade_no, spbill_create_ip, total_fee):
    ...
    stringSignTemp = '{0}&key={1}'.format(stringA, WECHAT_MCH_KEY)
...
def get_bodyData(openid, client_ip, price):
    body = BODY_DESC  # 商品描述
    notify_url = WECHAT_NOTIFY_URL  # 支付成功的回调地址
    nonce_str = getNonceStr()  # 随机字符串
    out_trade_no = getWxPayOrdrID()  # 商户订单号
    total_fee = str(int(float(price) * 100))  # 订单价格转换为分

    # 获取签名
    sign = paysign(WECHAT_APPID, body, WECHAT_MCH_ID, nonce_str, notify_url, openid, out_trade_no, client_ip, total_fee)

    bodyData = '<xml>'
    bodyData += '<appid>' + WECHAT_APPID + '</appid>'  # 小程序ID
    bodyData += '<body>' + body + '</body>'  # 商品描述
    bodyData += '<mch_id>' + WECHAT_MCH_ID + '</mch_id>'  # 商户号
    ...
    bodyData += '<trade_type>' + TRADE_TYPE + '</trade_type>'  # 交易类型
...
def dict_to_xml(dict_data):
    ...
    '''
    xml = ["<xml>"]
    for k, v in dict_data.items():  # Python 3 使用 items() 而不是 iteritems()

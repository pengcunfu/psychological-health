<template>
  <div class="banner-management">
    ...
    <div class="search-and-action-bar">
      <a-form layout="inline" :model="searchForm" @submit="handleSearch" class="search-form">
        <a-form-item label="标题">
          <a-input v-model:value="searchForm.title" placeholder="请输入轮播图标题" style="width: 200px;" />
        </a-form-item>
        <a-form-item>
    ...
    <!-- 轮播图列表 -->
    <a-table :columns="columns" :data-source="banners" :loading="loading" :pagination="pagination"
      @change="handleTableChange" row-key="id">
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'image_url'">
          <a-image :src="FileUploader.getFullImageUrl(record?.image_url)" :alt="record?.title || ''" width="120"
            height="60" style="object-fit: cover; border-radius: 4px;" :preview="true" />
        </template>
        ...
        <template v-else-if="column.key === 'action'">
          <a-space>
            ...
            <a-popconfirm title="确定要删除这个轮播图吗？" @confirm="deleteBanner(record.id)">
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" @ok="handleModalOk" @cancel="handleModalCancel"
      width="700px">
      <a-form ref="bannerFormRef" :model="bannerForm" :rules="bannerFormRules" layout="vertical">
        ...
        <a-form-item label="排序" name="sort_order">
          <a-input-number v-model:value="bannerForm.sort_order" placeholder="请输入排序值" style="width: 100%;" :min="0" />
        </a-form-item>

        <a-form-item label="轮播图" name="image_url">
          <a-upload v-model:file-list="fileList" :before-upload="beforeUpload" :custom-request="uploadImage"
            @remove="handleRemoveImage" list-type="picture-card" :max-count="1" accept="image/*" :show-upload-list="{
    ...
    <!-- 查看轮播图详情弹窗 -->
    <a-modal v-model:open="viewModalVisible" title="轮播图详情" :footer="null" width="600px">
      <div v-if="currentBanner" class="banner-detail">
        ...
        <div class="detail-image">
          <a-image :src="FileUploader.getFullImageUrl(currentBanner?.image_url)" :alt="currentBanner?.title || ''"
            width="100%" style="max-height: 300px; object-fit: contain;" :preview="true" />
        </div>

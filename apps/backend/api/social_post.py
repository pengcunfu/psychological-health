...
from form.social import SocialPostQueryForm, SocialPostCreateForm, SocialPostUpdateForm
from utils.json_result import JsonResult
from utils.validate import validate_args
from utils.auth_helper import is_manager_user, assert_current_user_id
...
def get_recommend_posts():
    ...
    return JsonResult.success(post_list)
...
def get_social_post_detail(post_id):
    ...
    if not is_manager and (post.status != 'published' or post.audit_status != 'approved'):
        ...
        if not current_user_id or post.user_id != current_user_id:
            return JsonResult.error('无权访问该帖子', 403)
    ...
    return JsonResult.success(post_data)
...
def create_social_post():
    ...
    try:
        ...
        return JsonResult.success(post.to_dict(), message='帖子发布成功')

    except Exception as e:
        db.session.rollback()
        return JsonResult.error(f'发布失败: {str(e)}')
...
def update_social_post(post_id):
    ...
    if post.user_id != current_user_id and not is_manager:
        return JsonResult.error('无权编辑该帖子', 403)
    ...
    try:
        ...
        return JsonResult.success(post.to_dict(), message='帖子更新成功')
...
def delete_social_post(post_id):
    ...
    # 权限检查：只有作者本人和管理员可以删除
    if post.user_id != current_user_id and not is_manager:
        return JsonResult.error('无权删除该帖子', 403)
    ...
    try:
        ...
        return JsonResult.success(message='帖子删除成功')

...
from form.social import SocialCommentQueryForm, SocialCommentCreateForm
...
def create_social_comment():
    ...
    if form.parent_id.data:
        ...
        if not parent_comment:
            return JsonResult.error('父评论不存在', 404)
    ...
    try:
        ...
        return JsonResult.success(comment.to_dict(), message='评论发布成功')
...
def delete_social_comment(comment_id):
    ...
    if comment.user_id != current_user_id and not is_manager_user():
        return JsonResult.error('无权删除该评论', 403)
    ...
    try:
        ...
        return JsonResult.success(message='评论删除成功')

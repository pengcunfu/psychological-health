...
def get_appointments(form):
    ...
    if not is_manager:
        if form.user_id.data and form.user_id.data != current_user_id:
            return json_error('权限不足，只能查看自己的预约', 403)
...
def get_appointment(appointment_id):
    ...
    return json_success(appointment.to_dict())
...
def create_appointment(form):
    ...
    if not is_manager_user():
        if form.user_id.data != current_user_id:
            return json_error('权限不足，只能为自己创建预约', 403)
    ...
    return json_success(appointment.to_dict(), '预约创建成功', 201)
...
def update_appointment(appointment_id, form):
    ...
    return json_success(appointment.to_dict(), '预约更新成功')
...
def delete_appointment(appointment_id):
    ...
    return json_success(None, '预约删除成功')
...
def confirm_appointment(appointment_id):
    ...
    try:
        ...
        db.session.commit()
        return json_success(appointment.to_dict(), '预约确认成功')
    except ValueError as e:
        return json_error(str(e), 400)
...
def complete_appointment(appointment_id):
    ...
    try:
        ...
        appointment.update_time = datetime.utcnow()
        db.session.commit()
        return json_success(appointment.to_dict(), '预约完成成功')
    except ValueError as e:
        return json_error(str(e), 400)
...
def cancel_appointment(appointment_id):
    ...
    try:
        ...
        appointment.update_time = datetime.utcnow()
        db.session.commit()
        return json_success(appointment.to_dict(), '预约取消成功')
    except ValueError as e:
        return json_error(str(e), 400)

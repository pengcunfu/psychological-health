...
def get_hot_topics():
    ...
    return json_success(topic_list)
...
def get_social_topic_detail(topic_id):
    ...
    return json_success(topic.to_dict())
...
def create_social_topic():
    ...
    try:
        ...
        return json_success(topic.to_dict(), message='话题创建成功')
...
def update_social_topic(topic_id):
    ...
    try:
        ...
        return json_success(topic.to_dict(), message='话题更新成功')
...
def delete_social_topic(topic_id):
    ...
    if topic.post_count > 0:
        return json_error('该话题下还有帖子，无法删除')

    try:
        db.session.delete(topic)
        db.session.commit()

        return json_success(message='话题删除成功')

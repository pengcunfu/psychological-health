...
class NotificationManager:
    ...
    def broadcast_notification(self,
        ...
        try:
            if not user_ids:
                return json_error("用户列表不能为空")

            if len(user_ids) > Config.MAX_BATCH_SIZE:
                return json_error(f"批量发送数量不能超过 {Config.MAX_BATCH_SIZE}")
            ...
            return json_error(f"批量发送通知失败: {str(e)}")
    ...
    def schedule_notification(self,
        ...
        try:
            if scheduled_time <= datetime.utcnow():
                return json_error("定时时间必须晚于当前时间")
            ...
            return json_error(f"创建定时通知失败: {str(e)}")
    ...
    def process_scheduled_notifications(self) -> Dict:
        ...
        try:
            ...
            if not scheduled_notifications:
                return json_success({'processed_count': 0}, "没有需要处理的定时通知")
            ...
            return json_error(f"处理定时通知失败: {str(e)}")
    ...
    def get_notification_statistics(self,
        ...
        try:
            ...
            return json_error(f"获取通知统计失败: {str(e)}")
    ...
    def create_notification_template(self,
        ...
        try:
            ...
            if existing:
                return json_error(f"模板代码 '{code}' 已存在")
            ...
            return json_success(template.to_dict(), "模板创建成功")

        except Exception as e:
            self.session.rollback()
            logger.error(f"创建通知模板失败: {str(e)}")
            return json_error(f"创建通知模板失败: {str(e)}")
    ...
    def update_user_notification_config(self,
        ...
        try:
            ...
            return json_success(config.to_dict(), "配置更新成功")

        except Exception as e:
            self.session.rollback()
            logger.error(f"更新用户通知配置失败: {str(e)}")
            return json_error(f"更新用户通知配置失败: {str(e)}")
    ...
    def get_user_notification_config(self, user_id: str) -> Dict:
        ...
        try:
            ...
            return json_success(config_dict)

        except Exception as e:
            logger.error(f"获取用户通知配置失败: {str(e)}")
            return json_error(f"获取用户通知配置失败: {str(e)}")

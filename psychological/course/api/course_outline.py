...
from ..models import CourseOutline, Course
...
from ..form import CourseOutlineQueryFormBase as CourseQueryForm, CourseOutlineQueryForm, CourseOutlineCreateForm, CourseOutlineUpdateForm
...
def get_course_outlines(form):
    ...
    return json_success([outline.to_dict() for outline in outlines])
...
def get_course_outline(outline_id):
    ...
    if not outline:
        return json_error('课程大纲不存在', 404)
    
    return json_success(outline.to_dict())
...
def create_course_outline(form):
    ...
    return json_success(outline.to_dict(), '创建课程大纲成功', 201)
...
def update_course_outline(outline_id, form):
    ...
    return json_success(outline.to_dict(), '更新课程大纲成功')
...
def delete_course_outline(outline_id):
    ...
    return json_success(None, '删除课程大纲成功')

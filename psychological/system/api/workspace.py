...
def get_workspace(workspace_id):
    ...
    if not workspace:
        return json_error('工作室不存在', 404)

    return json_success(workspace.to_dict())
...
def create_workspace(form):
    ...
    if form.validate_name_unique():
        return json_error('工作室名称已存在', 409)
    ...
    return json_success(workspace.to_dict(), '创建成功', 201)
...
def update_workspace(workspace_id, form):
    ...
    db.session.commit()
    return json_success(workspace.to_dict(), '更新成功')
